...
// const { t, locale: currentLocale, i18n } = useI18n({
//   legacy: false,
// });

const setLocale = () => {
  const currentLocale = ref();
  currentLocale.value = Cookies.get('locale');
  const newLocale = ref();
  newLocale.value = currentLocale.value == 'EN' ? 'AR' : 'EN';
  Cookies.set('locale', newLocale.value, { expires: 7 }) ;
  location.reload();
}
</script>
<template>
    ...
    <div class="offcanvas-body scrollbar-overlay px-x1 h-100" id="themeController">
      ...
      <div class="d-flex justify-content-between">
        <div class="d-flex align-items-start"><img class="me-2" :src="`/assets/img/icons/left-arrow-from-left.svg`"
            width="20" alt="" />
          <div class="flex-1">
            <h5 class="fs-0">Language</h5>
            <p class="fs--1 mb-0">AR or EN</p>
          </div>
        </div>
        <div class="form-check form-switch">
          <!-- <input class="form-check-input ms-0" id="mode-rtl" type="checkbox" data-theme-control="isRTL"
            @click="setLocale" /> -->
            <button type="button" class="btn btn-primary ms-0" @click="setLocale">{{ $t('anouther_language') }}</button>
        ...
        <div class="flex-1">
          ...
          <div>
            <select class="form-select form-select-sm" aria-label="Navbar position" data-theme-control="navbarPosition">
              <option value="vertical" data-page-url="modules/components/navs-and-tabs/vertical-navbar.html">Vertical
              </option>
              ...
              <option value="double-top" data-page-url="modules/components/navs-and-tabs/double-top-navbar.html">Double
                Top</option>
            </select>
          </div>
        </div>
